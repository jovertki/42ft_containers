...
int		main( void )
{
	...
	//START    <<< TRY THIS SHIET WITH FORWARD ITERS< CONST IS ODD
	const int size = 5;
	...
	*(it += 2) = 21;
	std::cout << "-----" << std::endl;
	for(TESTED_NAMESPACE::vector<TESTED_TYPE>::reverse_iterator itt = vct.rbegin(); itt != vct.rend(); itt++)
		std::cout << *itt << std::endl;

	std::cout << "-----" << std::endl;
	
	std::cout << "const_ite +=/-=: " << *(ite - 1) << " | " << *(ite) << std::endl;
	//END
